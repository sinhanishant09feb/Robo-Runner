...
public class PurpleWall : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider collision)
    {
        ...
        if(collision.gameObject.tag.Equals("Power"))
        {
            ...
            Player.GetComponent<MovementInput>().scor += 20;
            int no = PlayerPrefs.GetInt("PurpleWall");
            no++;
            PlayerPrefs.SetInt("PurpleWall", no);
            Instantiate(scorP20, this.transform.position, Quaternion.identity);
    ...
    public void Update()
    {
        if (col == 2)
        {
            ...
            Instantiate(purpleBlast, new Vector3(this.transform.position.x, this.transform.position.y + 2, this.transform.position.z), Quaternion.identity);
            scriptManager.GetComponent<SpawnWalls1>().chang2 = true;
            Destroy(this.gameObject);
        }
    }
